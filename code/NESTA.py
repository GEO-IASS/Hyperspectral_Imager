...
def NESTA(A,At,b,muf,delta,opts=None):
  ...
  opts,USV,userSet = setOpts(opts,'USV',dict());
  #if ~isempty(USV)
  if len(USV.keys()):
      #if isstruct(USV)
  ...
  if xplug is None or numpy.linalg.norm(xplug) < 1e-12:
      #if ~isempty(USV) && isempty(AAtinv)
      if len(USV.keys()) is not 0 and AAtinv is None:
          #AAtinv = Q*diag( s.^(-2) )*Q';
  ...
  #end
  if TypeMin.lower() == 'l1':
    mu0 = 0.9*(numpy.linalg.norm(Ux_ref,1)).max()
      ...
      if normU is None:
          ...
          if hasattr(U,'__call__'):
              ...
              if numpy.linalg.norm( UU - numpy.diag(numpy.diag(UU)),'fro') < 100*numpy.finfo(float).eps:
                  # this means the matrix is diagonal, so norm is easy:
                  #normU = sqrt( max(abs(diag(UU))) );
                  normU = math.sqrt( numpy.max(numpy.abs(numpy.diag(UU))) )
...
def my_normest(S,St,n,tol=1e-6, maxiter=20):
    ...
    x = x/e;
    e0 = 0;
    while numpy.abs(e-e0) > tol*e and cnt < maxiter:
       e0 = e;
       Sx = S(x);
       #if nnz(Sx) == 0
       if (Sx!=0).sum() == 0:
...
# opts = [];
  ...
  opts,AAtinv,userSet = setOpts(opts,'AAtinv',None);
  opts,USV,userSet = setOpts(opts,'USV',{});
  if len(USV.keys()) is not 0:
      #if isstruct(USV)
      Q = USV['U'];  # we can't use "U" as the variable name
                  # since "U" already refers to the analysis operator
      S = USV['S'];
...
def Perform_L1_Constraint(xk,mu,U,Ut):
    ...
    #uk = uk./max(mu,abs(uk));
    uk = uk / numpy.max([mu,numpy.linalg.norm(uk,1)])
    #val = real(uk'*fx);
